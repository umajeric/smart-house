...
/**
 * <p>
 * This GPIO provider implements the MCP23017 I2C GPIO expansion board as native Pi4J GPIO pins. More information about the board can be found here: *
 * http://ww1.microchip.com/downloads/en/DeviceDoc/21952b.pdf http://learn.adafruit.com/mcp230xx-gpio-expander-on-the-raspberry-pi/overview
 * </p>
 *
 * <p>
 ...
 *
 */
public class ExtendedMCP23017GpioProvider extends GpioProviderBase implements GpioProvider {
	private static final Logger logger = LoggerFactory.getLogger(ExtendedMCP23017GpioProvider.class);
	...
	private final int _providerAddress;

	public ExtendedMCP23017GpioProvider(int busNumber, int address) throws IOException {
	...
	public void setMode(Pin pin, PinMode mode) {
		...
		if (!pin.getSupportedPinModes().contains(mode)) {
			throw new InvalidPinModeException(pin, "Invalid pin mode [" + mode.getName() + "]; pin [" + pin.getName()
	...
	public void setState(Pin pin, PinState state) {
		// logger.info("setState called. PIN: {}, State: {}", pin, state);
		// validate
		if (hasPin(pin) == false) {
		...
		if (getPinCache(pin).getMode() != PinMode.DIGITAL_OUTPUT) {
			throw new InvalidPinModeException(pin, "Invalid pin mode on pin [" + pin.getName() + "]; cannot setState() when pin mode is ["
		...
		try {
			...
			if (pin != null && pin.getName() != null && !pin.getName().startsWith(PIN_NAME_PREFIX)) {
				logger.error("State saving cannot be performed as PIN name format is not recognized.");
				return;
			}
			/* remove pin name prefix to get only pin name (e.g.: A0, B2, etc.) */
			String name = pin.getName().replaceFirst(PIN_NAME_PREFIX, "");
			si.majeric.smarthouse.model.PinState newState = si.majeric.smarthouse.model.PinState.valueOf(state.toString());
			SwitchStateChangeEventListener.instance().notifyStateChanged(_providerAddress, name, newState);
		} catch (IOException ex) {
			throw new RuntimeException(ex);
		}
	...
	private class GpioStateMonitor extends Thread {
		...
		@Override
		public void run() {
			logger.info("GPIO Provider for address {} started...", _providerAddress);
			while (!shuttingDown) {
				try {
					synchronized (ExtendedMCP23017GpioProvider.class) {
						// only process for interrupts if any pin on port A is configured as an input pin
							...
							if (pinInterruptA > 0) {
								...
								for (Pin pin : MCP23017Pin.ALL_A_PINS) {
									...
									if ((pinInterruptA & pinAddressA) > 0) {
										// logger.info("INTERRUPT ON PIN [" + pin.getName() + "] (provider: " + _providerAddress + ")");
						...
						// only process for interrupts if any pin on port B is configured as an input pin
			...
			logger.info("GPIO Provider for address {} is shuting down...", _providerAddress);
		}
		...
		private void evaluatePinForChangeA(Pin pin, int state) {
			if (getPinCache(pin).isExported()) {
				...
				if ((state & pinAddress) != (currentStatesA & pinAddress)) {
					...
					// logger.info("<<< CHANGE >>> " + pin.getName() + " : " + state);
		...
		private void evaluatePinForChangeB(Pin pin, int state) {
			boolean exported = getPinCache(pin).isExported();
			if (exported) {
				// determine pin address
				int pinAddress = pin.getAddress() - GPIO_B_OFFSET;
		...
		private void dispatchPinChangeEvent(int pinAddress, PinState state) {
			...
			for (Pin pin : listeners.keySet()) {
				// logger.info("<<< DISPATCH >>> " + pin.getName() + " : " + state.getName() + ", provider: " + _providerAddress);
				...
				if (pin.getAddress() == pinAddress) {
					...
					for (PinListener listener : listeners.get(pin)) {
						// logger.info("<<< DISPATCH >>> for PIN: " + pin + ", provider: " + _providerAddress + ", LISTENER: "
						// + (listener != null ? listener.getClass() : ""));
